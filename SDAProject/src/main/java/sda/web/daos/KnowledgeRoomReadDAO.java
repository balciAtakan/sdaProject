...
public class KnowledgeRoomReadDAO
{
    ...
    public ArrayList<KnowledgeRoomView> getKnowledgeRooms() throws SDAException
    {
        ...
        String sql = "SELECT kr.*,kn_role.role_code ,kn_user.person_id, kn_user.username , krm.uuid as krm_uuid, krm.content, krm.owner, krm.modify_date" + "	FROM knowledge_room kr " + " left join knowledge_room_role kn_role on kr.uuid = kn_role.knowledge_room_id" + " left join knowledge_room_user kn_user on kr.uuid = kn_user.knowledge_room_id " + " left join knowledge_room_message krm on kr.uuid = krm.knowledge_room order by kr.date_create asc ";
                ...
                for (int i = 0; i < res.size(); i++)
                {
                    ...
                    if (item != null)
                    {
                        ...
                        if (!res.get(i).getHistory().isEmpty())
                        {
                            ...
                            if (message != null && item.getHistory().stream().noneMatch(
                                    a -> a.getUuid().equals(message.getUuid())))
                            {
